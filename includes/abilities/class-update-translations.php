...
class Update_Translations {
	...
	public static function register() {
		wp_register_ability(
			...
			array(
				...
				'description'         => __( 'Creates or updates translations for one or more originals in a project/locale, in a single batched call. Replicates GlotPress\'s own submission behaviour (validation, duplicate detection, and firing the same hooks other integrations rely on) rather than writing rows directly.', 'nakedcat-glotpress-abilities' ),
				'category'            => 'glotpress',
	...
	private static function output_schema() {
		return array(
			...
			'items' => array(
				...
				'required'   => array( 'original_id', 'result', 'translation_id', 'status', 'error_message' ),
			),
		);
	}
	...
	public static function execute( $input = array() ) {
		...
		foreach ( $items as $item ) {
			$results[] = self::process_item( $project, $translation_set, $locale, $item );
		}
